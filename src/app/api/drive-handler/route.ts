...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'findOrCreateFolder') {
      ...
      try {
        const q = `mimeType='application/vnd.google-apps.folder' and name='${folderName.replace(/'/g, "\\'")}' and trashed=false and '${parentFolderId === 'root' ? 'root' : parentFolderId}' in parents`;
        const listResponse = await drive.files.list({ q, fields: 'files(id, name)' });
        ...
        console.error('Error in findOrCreateFolder action:', error.response?.data || error.message);
        return NextResponse.json({ error: `Drive API error: ${error.response?.data?.error?.message || error.message}` }, { status: 500 });
      }
    } else if (action === 'uploadFile') {
      const { fileName, fileContentBase64, parentFolderId, mimeType = 'application/pdf' } = body;
      if (!fileName || !fileContentBase64 || !parentFolderId) {
        return NextResponse.json({ error: 'Missing fileName, fileContentBase64, or parentFolderId' }, { status: 400 });
      }
      ...
      try {
        ...
        console.error('Error in uploadFile action:', error.response?.data || error.message);
        return NextResponse.json({ error: `Drive API error: ${error.response?.data?.error?.message || error.message}` }, { status: 500 });
      }
    } else {

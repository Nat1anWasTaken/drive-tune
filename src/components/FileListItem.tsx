...
import type { PartStatus, ProcessedPart } from "@/types";
import {
  CheckCircle2,
  CloudUpload,
  Edit3,
...
export function PartListItem({
  ...
  const getDisplayPath = () => {
    if (!part.generatedFilename || !arrangementType || !arrangementName) {
      return part.generatedFilename || "Filename pending...";
    }
    // Display path is like: {Arrangement Type}/{Arrangement Name}/{Generated Filename}
    ...
    if (displayStr.length > 60) {
      // Truncate to show the end of the path
      return `.../${arrangementName}/${part.generatedFilename}`;
    }
    return displayStr;
  };
  ...
  const fullPathTitle = () => {
    ...
    return `${rootFolderName}/${arrangementType}/${arrangementName}/${part.generatedFilename}`;
  };
    ...
    <Card className="mb-2 shadow-sm border-border/70">
      <CardContent className="p-3">
        <div className="flex items-center justify-between mb-1">
          <span
            className="text-sm font-medium truncate max-w-[calc(100%-2.5rem)]"
            title={`Part Label: ${part.label} | Primary Instrumentation: ${part.primaryInstrumentation}`}
          >
            Part: {part.label}
          </span>
        ...
        {part.error && (
          <p className="text-destructive text-xs mt-1.5">{part.error}</p>
        )}
        ...
        {part.status === "done" &&
          ...
          rootFolderName && (
            <p
              className="text-xs text-green-600 mt-1.5 truncate"
              title={fullPathTitle()}
            >

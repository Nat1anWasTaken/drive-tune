...
export default function DriveTuneApp() {
  ...
  useEffect(() => {
    ...
    if (gisReady && !GOOGLE_CLIENT_ID) {
      toast({
        ...
        description: "Google Client ID is missing.",
      });
    }
  }, [gisReady, toast]);
  ...
  const handleConnectDrive = () => {
    if (!GOOGLE_CLIENT_ID) {
      toast({
        ...
        description: "Client ID is not set for client.",
      });
      return;
    }
    if (tokenClient) {
      ...
      toast({
        ...
        description: "Google Identity Service not ready.",
      });
    }
  };
  ...
  const handleSelectExistingFolder = () => {
    if (
      ...
      toast({
        variant: "destructive",
        title: "Error",
        description: "Drive not connected or Picker API not ready.",
      });
      return;
    }
    setIsSettingRootFolder(true);
    ...
    const picker = new (window as any).google.picker.PickerBuilder()
      ...
      .setCallback((data: any) => {
        ...
        if (data.action === (window as any).google.picker.Action.PICKED) {
          const doc = data[
            (window as any).google.picker.Response.DOCUMENTS
          ][0] as PickerDocument;
          setRootFolderDriveId(doc.id);
          setRootFolderDisplayId(doc.name);
          toast({
            title: "Folder Selected",
            description: `Root folder set to: "${doc.name}".`,
          });
        }
        setIsSettingRootFolder(false);
      })
  ...
  const handleFileChangeForArrangement = (
    arrangementId: string,
    ...
    if (event.target.files && event.target.files.length > 0) {
      ...
      updateArrangement(arrangementId, {
        ...
        statusMessage: `${pdfFiles.length} file(s) ready.`,
      });
    }
  };
  ...
  const processArrangement = async (arrangement: Arrangement) => {
    if (!rootFolderDriveId || !rootFolderDisplayId) {
      updateArrangement(arrangement.id, {
        status: "error",
        statusMessage: "Root Drive folder not set.",
    ...
    if (!arrangement.files || arrangement.files.length === 0) {
      updateArrangement(arrangement.id, {
        status: "error",
        statusMessage: "No files uploaded for this arrangement.",
        error: "No file(s).",
      });
      return;
    }
    ...
    try {
      if (arrangement.files.length > 1) {
        updateArrangement(arrangement.id, {
          status: "merging_files",
          statusMessage: "Merging PDF files...",
        });
        try {
          fileToProcess = await mergePdfs(arrangement.files);
          updateArrangement(arrangement.id, {
            statusMessage: "PDFs merged. Reading file...",
          });
        } catch (mergeError: any) {
          console.error("Error merging PDFs:", mergeError);
          updateArrangement(arrangement.id, {
            status: "error",
            statusMessage: `Error merging PDFs: ${mergeError.message}`,
      ...
      updateArrangement(arrangement.id, {
        ...
        statusMessage: "Extracting metadata with AI...",
      });
      const metadata: ExtractedMusicSheetMetadata =
        await extractMusicSheetMetadata({ musicSheetDataUri: dataUri });
      ...
      if (metadata.parts.some((p) => !p.primaryInstrumentation)) {
        throw new Error(
          "AI failed to provide primary instrumentation for one or more parts."
        );
      }
      ...
      if (!conceptualDirResult.success || !conceptualDirResult.directoryPath) {
        throw new Error(
          "AI failed to determine conceptual directory structure."
        );
      }

      const arrangementTypeFolderDriveId = await findOrCreateFolderAPI(
      ...
      const initialProcessedParts: ProcessedPart[] = metadata.parts.map(
        (partInfo, index) => ({
          ...partInfo,
          id: `${arrangement.id}-part-${index}-${partInfo.label.replace(
            /\s+/g,
            "-"
          )}`,
      ...
      for (const part of initialProcessedParts) {
        try {
          ...
          const partErrorMessage =
            partError.message || "Unknown error processing part.";
          updatePartStatus(
            arrangement.id,
            part.id,
            "error",
      ...
      setArrangements((prevArrangements) => {
        const currentArr = prevArrangements.find(
          (a) => a.id === arrangement.id
        );
        if (currentArr) {
          const allPartsDoneOrError = currentArr.processedParts.every(
            (p) => p.status === "done" || p.status === "error"
          );
          if (allPartsDoneOrError) {
            const hasErrors = currentArr.processedParts.some(
              (p) => p.status === "error"
            );
            ...
            if (!hasErrors) {
              toast({
                title: "Arrangement Organized",
                description: `"${currentArrangementName}" uploaded to Drive.`,
              });
            } else {
              toast({
                variant: "default",
                title: "Arrangement Processed",
                description: `"${currentArrangementName}" processed. Check part statuses.`,
                duration: 5000,
              });
            }
            return prevArrangements.map((a) =>
              a.id === arrangement.id
                ? { ...a, status: finalStatus, statusMessage: finalMessage }
                : a
            );
          }
        }
      ...
      console.error(
        "Error processing arrangement:",
        currentArrangementName,
        error
      );
      const errorMessage =
        error.message ||
        "An unknown error occurred during arrangement processing.";
      updateArrangement(arrangement.id, {
        status: "error",
        statusMessage: `Error processing "${currentArrangementName}": ${errorMessage}`,
        error: errorMessage,
      });
      toast({
        variant: "destructive",
        title: `Error processing ${currentArrangementName}`,
  ...
  const handleProcessAllReadyArrangements = async () => {
    if (!rootFolderDriveId) {
      toast({
        ...
        title: "Error",
        description:
          "Please select and confirm a root folder in Google Drive first.",
      });
      return;
    }
    ...
    if (readyArrangements.length === 0) {
      toast({
        title: "Info",
        description: "No new arrangements ready to process.",
      });
      return;
    }
    ...
    for (const arrangement of readyArrangements) {
      const currentArrangementToProcess = arrangements.find(
        (a) => a.id === arrangement.id
      );
    ...
    <div className="container mx-auto p-4 md:p-8 min-h-screen flex flex-col items-center bg-background">
      <header className="mb-8 text-center">
        ...
        <p className="text-xl text-muted-foreground">
          Organize Your Music Sheets in Google Drive
        </p>
      </header>

      <div className="w-full max-w-3xl space-y-6">
        {!GOOGLE_CLIENT_ID || !GOOGLE_PICKER_API_KEY ? (
        ...
        <Card className="shadow-lg">
          <CardHeader>
            ...
            <CardDescription>
              Sign in with Google and authorize access to your Drive.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {!isDriveConnected ? (
        ...
        {isDriveConnected && (
          <Card className="shadow-lg">
            <CardHeader>
              <CardTitle className="flex items-center">
                <FolderOpenDot className="mr-2 h-6 w-6 text-primary" />
                Step 2: Select or Create Root Folder
              </CardTitle>
              <CardDescription>
                Choose a main folder in Drive for organization. Either select an
                existing one or create one by name.
              </CardDescription>
            </CardHeader>
              ...
              <div className="space-y-2">
                ...
                <div className="flex space-x-2">
                  ...
                  <Button
                    ...
                    disabled={
                      isSettingRootFolder ||
                      !tempRootFolderName.trim() ||
                      !isDriveConnected ||
                      !!rootFolderDriveId
                    }
                    className="whitespace-nowrap"
                  >
                    {isSettingRootFolder && !rootFolderDriveId ? (
                      <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                    ) : null}
                    Set by Name
                  </Button>
            ...
            <CardFooter>
              {rootFolderDriveId ? (
                <div className="flex items-center text-green-600 p-3 bg-green-50 border border-green-200 rounded-md w-full">
                  <CheckCircle className="mr-2 h-5 w-5" />
                  <span>
                    Root folder selected: "{rootFolderDisplayId}" (ID:{" "}
                    {rootFolderDriveId.substring(0, 10)}...)
                  </span>
                </div>
              ) : (
          ...
          <Card className="shadow-lg">
            <CardHeader>
              <CardTitle className="flex items-center">
                ...
                Step 3: Add & Organize Arrangements
              </CardTitle>
              <CardDescription>
                Upload PDF(s) per arrangement. They'll be merged, metadata
                extracted, parts split, named, and organized into:{" "}
                {rootFolderDisplayId}/[Arrangement Type]/[Arrangement Name] -
                [Part Name].pdf
              </CardDescription>
            </CardHeader>
            <CardContent>
                    ...
                    <div className="space-y-4 p-2">
                      {arrangements.map((arrangement) => (
                        <ArrangementListItem
                          ...
                          updateArrangementName={(id, newName) =>
                            updateArrangement(id, { name: newName })
                          }
            ...
            <CardFooter>
              ...
              >
                ...
                {isProcessingGlobal
                  ...
                  : `Organize ${numReadyToProcess} Ready Arrangement(s) in Drive`}
              </Button>
            </CardFooter>
          </Card>
        )}
          ...
          <Card className="shadow-lg mt-6 bg-green-50 border-green-200">
            <CardHeader>
              <CardTitle className="flex items-center text-green-700">
                ...
                All Submitted Arrangements Processed!
              </CardTitle>
            </CardHeader>
            <CardContent>
              <p className="text-green-600">
                You can add more arrangements or clear the list to start fresh.
              </p>
              <Button
